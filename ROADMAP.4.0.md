...
`unified-ble-manager@4.0.0-alpha.30` is published under npm's `next` dist-tag
...
or examples. The alpha has no 3.x compatibility layer. `v4.0.0-alpha.29` is the
...
evidence record linking alpha.30's published artifact to a passed physical-radio
