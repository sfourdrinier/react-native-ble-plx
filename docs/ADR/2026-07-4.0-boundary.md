...
floor nor authorizes another byte path. The accepted JSI direction is recorded
by [`spikes/rn-jsi-binary`](../../spikes/rn-jsi-binary/README.md).
...
- The Android installer is `installExecutionRuntime` on the negotiated control
  module. It installs exactly one `__unifiedBleNativeProtocolV1` host object
...
a narrow preload proxy for IPC protocol v1; it never receives Node, native
...
boundary declarations are deleted or mechanically replaced before G1.
