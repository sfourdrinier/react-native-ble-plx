...
`unified-ble-manager@4.0.0-alpha.36` is the published Experimental prerelease,
...
must not be read as Preview-or-higher evidence for the alpha.36 prerelease.
...
| Web Bluetooth | Chooser-specific backend, authorization semantics, notifications, lifecycle hardening, browser-safe bundle, TCK, public scenarios, and clean-baseline physical validation harness implemented | Browser build, TCK, declared live Chromium proof | Retained physical Web Bluetooth chooser/connect/discover/read/notify/cleanup evidence on declared browser/OS |
