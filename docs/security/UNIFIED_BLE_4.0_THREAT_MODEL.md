...
`unified-ble-manager` is an open-source, multi-host Bluetooth Low Energy central library. It is being rebuilt as a 4.0 clean baseline; the current React Native, web, Electron, native-addon, and desktop code is transitional implementation evidence rather than a published 4.0 support promise. The durable product boundary is a host-neutral public contract with explicit host backends, typed capabilities and limitations, byte-oriented GATT operations, deterministic lifecycle semantics, and release evidence. This model applies to the whole repository and its release workflow, not to a particular diff or a list of current findings.
...
Primary runtime surfaces are the public manager and backend contract (`src/`, especially `src/port/BlePort.ts`, `src/port/PortBleManager.ts`, `src/BleManager.ts`, and `src/ConnectionManager.ts`); React Native Android and Apple native bridges; the optional Apple restoration path; browser Web Bluetooth; Electron main-process native backends; BlueZ D-Bus, CoreBluetooth N-API, and WinRT adapters; and Expo configuration plugins. Privileged workflow surfaces are package builds, native-addon builds, CI/release publishing, third-party backend SDK/TCK integration, and the evidence-manifest system. Examples, fixtures, benchmarks, diagnostics, and labs are not production authority, but can become security-relevant if they are packaged, run in CI with credentials, or used to make support claims.
...
The manager, port, discovery/profile helpers, connection manager, operation queue, byte codec, and error mapping accept inputs that eventually cause radio actions or data delivery. Relevant classes include malformed identifiers/UUIDs, oversized or adversarial payloads, confused handle ownership, re-entrant callbacks, cross-client cancellation, stale generations, notification floods, scan amplification, queue/resource exhaustion, and error paths that falsely report success. Existing direction in `src/DeviceOperationQueue.ts`, `src/BleManager.ts`, `src/ConnectionManager.ts`, and the 4.0 plan emphasizes per-device serialization, cancel epochs, destroy-time rejection, typed errors, explicit unsupported behavior, and host-owned reconnect policy. Future contract/TCK work must prove these semantics under overlap, cancellation, timeout, reconnect, and flood conditions rather than only under happy-path reads and writes.
...
The React Native bridge carries device identifiers, Base64/byte conversions in the transitional tree, transaction/cancellation handles, permission requests, background-service actions, and native events. Android manifest and foreground-service configuration affects location-adjacent scanning, Bluetooth permissions, notification visibility, and a non-exported connected-device service. Apple code owns CoreBluetooth callbacks and optional restoration state. `plugin/src/withBLE.ts`, `withBLEAndroidManifest.ts`, and `withBLEAndroidForegroundService.ts` default background/restoration off, track plugin-owned manifest entries, preserve other app permissions on removal, and declare the service non-exported. `ios/Restoration/BlePlxRestorationAdapter.swift` reports restored state and adopts the OS-restored central rather than reconnecting or constructing a second owner; `BlePlxRestorationState.swift` serializes one-time handoff.
...
`src/hosts/web.ts` is constrained by the browser chooser and secure context, not a background scanner. Its request-option shaping rejects ambiguous filter/accept-all combinations and empty granted-service sets; error mapping distinguishes chooser cancellation, policy/permission, and identifier failures. Threats include untrusted filter/service input expanding access, user-gesture or origin-policy mistakes, previously granted devices, malformed DOM/GATT values, disconnect event races, notification floods, and accidental logging/persistence of browser-visible device IDs. The backend must keep chooser constraints explicit, restrict requested services to consumer need, use a secure context, copy `DataView` bytes, and deliver bounded, removable subscriptions. Browser permission does not authenticate a peripheral or authorize sensitive product actions.

### Electron, native addon, BlueZ, and WinRT

Electron's intended model is main-only radio ownership. The example already demonstrates `contextIsolation`, `sandbox`, disabled Node integration, a narrow context bridge, native-required failure behavior, and device-ID shape/known-device checks (`example-electron/main.js`, `preload.js`, and `deviceIdGuard.js`), but examples are not a substitute for the final versioned protocol. The production design must bind every request and event to the authorized `webContents`/window/session, validate schemas and resource quotas at main, and define reload, crash, close, and multi-window ownership. A compromised renderer must never obtain general Node APIs, invoke arbitrary main methods, enumerate another window's devices, or retain radio subscriptions after its owner dies.

CoreBluetooth's N-API path (`native/electron/corebluetooth/` and `src/hosts/native/corebluetooth/CoreBluetoothBlePort.ts`) crosses JavaScript, C++/Objective-C++, dispatch queues, and CoreBluetooth. It must treat add-on path/ABI/surface validation, native exceptions, callback lifetime, copied buffers, asynchronous completion, listener isolation, and destruction as security boundaries. A successful binary load or ABI build is not proof of a complete/live backend. BlueZ (`src/hosts/native/bluez/BluezBlePort.ts`) uses an optional system D-Bus package and must validate object paths, interfaces, properties, `ay` payloads, signals, bus errors, timeouts, and teardown; it cannot equate package installation with a usable system bus. WinRT remains an optional native boundary with the same ABI/capability/fail-closed requirements.
