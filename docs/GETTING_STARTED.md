...
`unified-ble-manager@4.0.0-alpha.37` is the current published 4.0 alpha package.
...
Experimental, and no current evidence record binds the published alpha.37
...
L2/L3 evidence only; alpha.37 makes no Windows live-radio claim.
...
`pnpm add unified-ble-manager@4.0.0-alpha.37 expo@^57.0.0`; the plugin uses
