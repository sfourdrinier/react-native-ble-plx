...
`unified-ble-manager@4.0.0-alpha.30` was published from
`v4.0.0-alpha.30` by the protected GitHub Actions trusted-publishing workflow.
...
[GitHub Release](https://github.com/sfourdrinier/react-native-ble-plx/releases/tag/v4.0.0-alpha.30)
is a prerelease with notes generated from the alpha.30 section of
[`CHANGELOG.md`](CHANGELOG.md).

`v4.0.0-alpha.29` is the previous published prerelease. It is not the
current prerelease and does not bind alpha.30's package or evidence state.
...
WinRT compile and ABI checks are L2/L3 evidence only; alpha.30 makes no Windows
live-radio claim.
...
| --- | --- | --- | --- |
| Hyphenated SemVer prerelease, such as `4.0.0-alpha.30` | `next` | prerelease | Pin the exact version for reproducible evaluation; `@next` is mutable. |
...
```sh
npm view unified-ble-manager@4.0.0-alpha.30 version dist-tags dist.integrity dist.attestations _npmUser --json
```

Then cross-check the matching tag and GitHub Release:

```sh
gh release view v4.0.0-alpha.30 --repo sfourdrinier/react-native-ble-plx --json tagName,isPrerelease,publishedAt,url
```

After alpha.30 publication, npm must report the exact version, `next`, integrity,
a SLSA provenance attestation, and GitHub Actions trusted publisher; GitHub must
report the matching `v4.0.0-alpha.30` tag with `isPrerelease: true`. These checks verify release
