...
`unified-ble-manager@4.0.0-alpha.22` was published from
`v4.0.0-alpha.22` by GitHub Actions trusted publishing. The published npm
...
is a prerelease and its notes are generated from the alpha.22 section of
...
not bind alpha.22's package artifact to a passed physical-radio scenario. No
...
| --- | --- | --- | --- |
| Hyphenated SemVer prerelease, such as `4.0.0-alpha.22` | `next` | prerelease | Pin the exact version for reproducible evaluation; `@next` is mutable. |
...
```sh
gh release view v4.0.0-alpha.22 --repo sfourdrinier/react-native-ble-plx --json tagName,isPrerelease,publishedAt,url
```

For alpha.22, npm must report the exact version, `next`, integrity, a SLSA
provenance attestation, and GitHub Actions trusted publisher; GitHub must report
the matching `v4.0.0-alpha.22` tag with `isPrerelease: true`. These checks
...
At the time of alpha.22, GitHub private vulnerability reporting is disabled and
