...
`unified-ble-manager@4.0.0-alpha.32` was published from
`v4.0.0-alpha.32` by the protected GitHub Actions trusted-publishing workflow.
...
| --- | --- | --- | --- |
| Hyphenated SemVer prerelease, such as `4.0.0-alpha.32` | `next` | prerelease | Pin the exact version for reproducible evaluation; `@next` is mutable. |
...
```sh
gh release view v4.0.0-alpha.32 --repo sfourdrinier/react-native-ble-plx --json tagName,isPrerelease,publishedAt,url
```
...
report the matching `v4.0.0-alpha.32` tag with `isPrerelease: true`. These checks verify release
