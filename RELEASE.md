...
`unified-ble-manager@4.0.0-alpha.21` was published from
`v4.0.0-alpha.21` by GitHub Actions trusted publishing. The published npm
...
is a prerelease and its notes are generated from the alpha.21 section of
...
not bind alpha.21's package artifact to a passed physical-radio scenario. No
...
| --- | --- | --- | --- |
| Hyphenated SemVer prerelease, such as `4.0.0-alpha.21` | `next` | prerelease | Pin the exact version for reproducible evaluation; `@next` is mutable. |
...
```sh
gh release view v4.0.0-alpha.21 --repo sfourdrinier/react-native-ble-plx --json tagName,isPrerelease,publishedAt,url
```

For alpha.21, npm must report the exact version, `next`, integrity, a SLSA
provenance attestation, and GitHub Actions trusted publisher; GitHub must report
the matching `v4.0.0-alpha.21` tag with `isPrerelease: true`. These checks
...
At the time of alpha.21, GitHub private vulnerability reporting is disabled and
