...
public class BleClientManager : NSObject {
...
#else
        // CoreBluetooth state restoration is API_UNAVAILABLE(tvos); ignore the restore key.
        _ = restoreIdentifierKey
        manager = BluetoothManager(queue: queue)
#endif
    ...
    // iOS-only: CoreBluetooth state restoration (RestoredState) is unavailable on tvOS.
#if os(iOS)
