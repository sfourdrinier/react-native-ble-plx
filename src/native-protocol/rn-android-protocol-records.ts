...
export function nativePeerIdForCommand(command: NativeProtocolRecord): string | null {
  ...
  if (kind === 'read' || kind === 'write' || kind === 'subscribe' || kind === 'unsubscribe') {
    const service = requiredRecord(
      requiredRecord(command, 4, 'rn-android-boundary.command.characteristic'),
      1,
      'rn-android-boundary.command.service'
    )
...
export function advertisementFromRecord(record: NativeProtocolRecord): CoreBluetoothAdvertisement {
  if (record.kind !== 'advertisement') {
    throw contractError('protocol.malformed', 'boundary', 'rn-android-boundary.advertisement')
  }
  const rssiField = record.fields.find(candidate => candidate.id === 6)?.value
  const serviceUuids = record.fields.find(candidate => candidate.id === 10)?.value
  return Object.freeze({
    nativePeerId: requiredString(record, 1, 'rn-android-boundary.advertisement.peer'),
    localName: optionalString(record, 5),
    rssi: typeof rssiField === 'number' && Number.isSafeInteger(rssiField) ? rssiField : null,
    serviceUuids:
      Array.isArray(serviceUuids) && serviceUuids.every(item => typeof item === 'string') ? serviceUuids : null
  })
}
