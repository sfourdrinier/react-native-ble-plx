...
describe('4.0 documentation honesty', () => {
  ...
  test('migration documents the current v4 package without inventing a compatibility path', () => {
    ...
    expect(migration).toContain('unified-ble-manager@4.0.0-alpha.21')
  ...
  test('public README provides only current alpha.21 construction and plugin guidance', () => {
    const readme = read('README.md')
    const changelog = read('CHANGELOG.md')

    expect(readme).toContain('unified-ble-manager@4.0.0-alpha.21')
    expect(readme).toContain('pnpm add unified-ble-manager@4.0.0-alpha.21')
  ...
  test('published alpha documentation preserves exact release, evidence, and deferral boundaries', () => {
    ...
    expect(release).toContain('v4.0.0-alpha.21')
    expect(release).toContain('GitHub Actions trusted publishing')
    expect(release).toContain('SLSA provenance')
    expect(platforms).toContain('No current evidence record binds the published alpha.21 artifact')
