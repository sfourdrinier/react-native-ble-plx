...
describe('4.0 documentation honesty', () => {
  ...
  test('migration documents the current v4 package without inventing a compatibility path', () => {
    ...
    expect(migration).toContain('unified-ble-manager@4.0.0-alpha.22')
  ...
  test('public README provides only current alpha.22 construction and plugin guidance', () => {
    const readme = read('README.md')
    const changelog = read('CHANGELOG.md')

    expect(readme).toContain('unified-ble-manager@4.0.0-alpha.22')
    expect(readme).toContain('pnpm add unified-ble-manager@4.0.0-alpha.22')
  ...
  test('published alpha documentation preserves exact release, evidence, and deferral boundaries', () => {
    ...
    expect(release).toContain('v4.0.0-alpha.22')
    expect(release).toContain('GitHub Actions trusted publishing')
    expect(release).toContain('SLSA provenance')
    expect(platforms).toContain('No current evidence record binds the published alpha.22 artifact')
