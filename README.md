...
`unified-ble-manager@4.0.0-alpha.30` is the current published 4.0 prerelease for
...
`v4.0.0-alpha.30` was published by GitHub Actions trusted publishing under
...
current prerelease or an alpha.30 evidence record.

The package and deterministic contract surface are **Experimental**. No current
evidence record binds the published alpha.30 artifact to a hardware-backed
...
evidence only; alpha.30 makes no Windows live-radio claim.
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.30
```

`next` is the mutable prerelease dist-tag. Pin the exact alpha.30 version you
