...
`unified-ble-manager@4.0.0-alpha.24` is the current public 4.0 prerelease for
...
`v4.0.0-alpha.24` is published to npm under the `next` dist-tag and has a
...
evidence record binds the published alpha.24 artifact to a hardware-backed
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.24
```

`next` is the mutable prerelease dist-tag. It currently resolves to alpha.24,
...
For alpha.24, the result must identify the exact version, `next` dist-tag,
...
that still needs host-specific evidence.
