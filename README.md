...
`unified-ble-manager@4.0.0-alpha.23` is the current public 4.0 prerelease for
...
`v4.0.0-alpha.23` is published to npm under the `next` dist-tag and has a
...
evidence record binds the published alpha.23 artifact to a hardware-backed
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.23
```

`next` is the mutable prerelease dist-tag. It currently resolves to alpha.23,
...
For alpha.23, the result must identify the exact version, `next` dist-tag,
