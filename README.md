...
`unified-ble-manager@4.0.0-alpha.37` is the current published 4.0 prerelease for
...
`v4.0.0-alpha.37` was published by GitHub Actions trusted publishing under
...
current prerelease or an alpha.37 evidence record.

The package and deterministic contract surface are **Experimental**. No current
evidence record binds the published alpha.37 artifact to a hardware-backed
...
evidence only; alpha.37 makes no Windows live-radio claim.
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.37
```

`next` is the mutable prerelease dist-tag. Pin the exact alpha.37 version you
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.37 expo@^57.0.0
```
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.37 dbus-next@^0.10.2
```
