...
current prerelease or an alpha.34 evidence record.

The package and deterministic contract surface are **Experimental**. No current
evidence record binds the published alpha.34 artifact to a hardware-backed
...
evidence only; alpha.34 makes no Windows live-radio claim.
