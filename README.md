...
`unified-ble-manager@4.0.0-alpha.36` is the current published 4.0 prerelease for
...
`v4.0.0-alpha.36` was published by GitHub Actions trusted publishing under
...
current prerelease or an alpha.36 evidence record.

The package and deterministic contract surface are **Experimental**. No current
evidence record binds the published alpha.36 artifact to a hardware-backed
...
evidence only; alpha.36 makes no Windows live-radio claim.
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.36
```

`next` is the mutable prerelease dist-tag. Pin the exact alpha.36 version you
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.36 expo@^57.0.0
```
...
```sh
pnpm add unified-ble-manager@4.0.0-alpha.36 dbus-next@^0.10.2
```
